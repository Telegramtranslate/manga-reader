...
const state={currentView:"home",previousView:"home",latest:[],recommended:[],popular:[],catalogItems:[],ongoingItems:[],topItems:[],scheduleItems:[],searchResults:[],sortingOptions:[],typeOptions:[],favorites:[],authUser:null,featured:null,searchTimer:null,searchAbort:null,searchQuery:"",catalogPage:0,catalogTotal:0,catalogHasMore:false,catalogSort:"FRESH_AT_DESC",catalogType:"",ongoingPage:0,ongoingTotal:0,ongoingHasMore:false,topPage:0,topTotal:0,topHasMore:false,referencesLoaded:false,homeLoaded:false,catalogLoaded:false,ongoingLoaded:false,topLoaded:false,scheduleLoaded:false,currentAnime:null,currentEpisode:null,currentQuality:"720",currentSource:"anilibria",manifestBlobUrl:null,hls:null};
...
function renderCatalogControls(){
  els.catalogSort.innerHTML="";els.catalogType.innerHTML='<option value="">Все форматы</option>';
...
async function loadCatalog(options={}){
  ...
  if(reset)updateGrid(els.catalogGrid,state.catalogItems,"Каталог пуст.");else updateGrid(els.catalogGrid,releases,"Каталог пуст.",{append:true,offset:state.catalogItems.length-releases.length});
...
async function loadOngoing(options={}){
  ...
  els.ongoingMoreBtn.disabled=true;const payload=await fetchJson("/anime/catalog/releases",buildCatalogParams(nextPage,{"f[publish_statuses]":"IS_ONGOING"}),{ttl:120000});const releases=buildReleases(payload);const pagination=extractPagination(payload);
...
async function loadTop(options={}){
  ...
  els.topMoreBtn.disabled=true;const payload=await fetchJson("/anime/catalog/releases",{page:nextPage,limit:GRID_PAGE_SIZE,"f[sorting]":"RATING_DESC"},{ttl:120000});const releases=buildReleases(payload);const pagination=extractPagination(payload);
...
function buildQualityOptions(episode){const options=[{key:"1080",label:"1080p",url:episode.hls_1080},{key:"720",label:"720p",url:episode.hls_720},{key:"480",label:"480p",url:episode.hls_480}].filter((item)=>item.url);if(!options.some((item)=>item.key===state.currentQuality)){const preferred=options.find((item)=>item.key==="720")||options[0];state.currentQuality=preferred?preferred.key:"720";}return options;}
...
async function selectEpisode(episode,options={}){if(!state.currentAnime)return;if(!options.preserveSource)state.currentSource="anilibria";state.currentEpisode=episode;renderEpisodes(state.currentAnime);renderSourceSwitch(state.currentAnime);showVideoSurface();stopExternalPlayer();const qualities=renderQualityButtons(episode);const selected=qualities.find((q)=>q.key===state.currentQuality)||qualities[0];els.playerTitle.textContent=`${episode.ordinal} серия${episode.name?` • ${episode.name}`:""}`;if(!selected){destroyPlayer();els.playerNote.textContent="У этой серии пока нет доступного потока.";return;}els.playerNote.textContent="Поток загружается через ваш домен, поэтому мобильный браузер не упирается в CORS внешнего CDN.";try{await attachPlayer(selected.url);els.player.play().catch(()=>{});}catch(error){console.error(error);els.playerNote.textContent="Не удалось загрузить поток. Попробуйте другое качество или другую серию.";}}
function switchSource(sourceId){if(!state.currentAnime)return;state.currentSource=sourceId;renderSourceSwitch(state.currentAnime);if(sourceId==="external"&&state.currentAnime.externalPlayer){showExternalSurface(state.currentAnime.externalPlayer);els.qualitySwitch.innerHTML="";els.playerTitle.textContent="Другие озвучки";els.playerNote.textContent="Если внешний источник поддерживает AniDub, DEEP, Studio Band или другие переводы, выбирайте их внутри этого плеера.";return;}if(state.currentEpisode){selectEpisode(state.currentEpisode,{preserveSource:true}).catch(console.error);return;}if(state.currentAnime.episodes.length){selectEpisode(state.currentAnime.episodes[0],{preserveSource:true}).catch(console.error);return;}destroyPlayer();stopExternalPlayer();els.qualitySwitch.innerHTML="";els.playerTitle.textContent="Серии отсутствуют";els.playerNote.textContent="Для этого релиза пока нет опубликованных эпизодов.";}
async function openRelease(alias,options={}){const payload=await fetchJson(`/anime/releases/${encodeURIComponent(alias)}`,null,{ttl:DETAIL_TTL});const release=buildRelease(payload);state.currentAnime=release;state.currentEpisode=null;state.currentQuality="720";state.currentSource="anilibria";renderDetails(release);if(options.updateHash!==false)syncHash(`#anime/${encodeURIComponent(alias)}`);openDrawer();if(release.episodes.length){await selectEpisode(release.episodes[0]);return;}if(release.externalPlayer){switchSource("external");return;}destroyPlayer();stopExternalPlayer();els.qualitySwitch.innerHTML="";els.playerTitle.textContent="Серии отсутствуют";els.playerNote.textContent="Для этого релиза пока нет опубликованных эпизодов.";}
...
async function refreshAll(){responseCache.clear();requestCache.clear();state.homeLoaded=false;state.catalogLoaded=false;state.ongoingLoaded=false;state.topLoaded=false;state.scheduleLoaded=false;state.referencesLoaded=false;if(state.searchAbort){state.searchAbort.abort();state.searchAbort=null;}await loadReferences(true);await loadHome(true);await ensureViewLoaded(state.currentView);}
function registerServiceWorker(){if(!("serviceWorker"in navigator))return;window.addEventListener("load",()=>{navigator.serviceWorker.register("./sw.js?v=4",{updateViaCache:"none"}).catch(()=>{});});}
