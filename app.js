...
function isAdminUser(){const email=String(state.authUser?.email||"").trim().toLowerCase();return email==="serikovmaksim94@gmail.com";}
...
function renderSourceSwitch(release){els.sourceSwitch.innerHTML="";const fragment=document.createDocumentFragment();buildSourceList(release).forEach((source)=>{const button=document.createElement("button");button.type="button";button.dataset.sourceId=source.id;button.className=`source-btn${state.currentSource===source.id?" is-active":""}`;button.innerHTML=`<strong>${escapeHtml(source.title)}</strong><small>${escapeHtml(source.note)}</small>`;button.addEventListener("click",()=>switchSource(source.id));fragment.appendChild(button);});els.sourceSwitch.appendChild(fragment);}
...
function syncRenderedSourceState(){els.sourceSwitch.querySelectorAll(".source-btn").forEach((button)=>button.classList.toggle("is-active",button.dataset.sourceId===state.currentSource));}
function syncRenderedEpisodeState(){const currentEpisodeId=String(state.currentEpisode?.id||"");els.episodesList.querySelectorAll(".episode-btn").forEach((button)=>button.classList.toggle("is-active",button.dataset.episodeId===currentEpisodeId));}
...
function registerServiceWorker(){if(!("serviceWorker"in navigator))return;window.addEventListener("load",()=>{navigator.serviceWorker.register("./sw.js?v=16",{updateViaCache:"none"}).catch(()=>{});});}
...
renderEpisodes=function(release){els.episodesList.innerHTML="";if(!release.episodes.length){els.episodesList.appendChild(createEmptyState("РЈ СЌС‚РѕРіРѕ СЂРµР»РёР·Р° РїРѕРєР° РЅРµС‚ РѕРїСѓР±Р»РёРєРѕРІР°РЅРЅС‹С… СЃРµСЂРёР№."));return;}const fragment=document.createDocumentFragment();release.episodes.forEach((episode)=>{const button=document.createElement("button");button.type="button";button.className=`episode-btn${state.currentEpisode?.id===episode.id?" is-active":""}`;button.dataset.episodeId=episode.id||"";button.dataset.ordinal=String(episode.ordinal||"");button.innerHTML=`<strong>${escapeHtml(`${episode.ordinal} СЃРµСЂРёСЏ`)}</strong><span>${escapeHtml(episode.name||"Р‘РµР· РЅР°Р·РІР°РЅРёСЏ")}</span><small>${escapeHtml(formatEpisodeDuration(episode.duration)||"Р”Р»РёС‚РµР»СЊРЅРѕСЃС‚СЊ РЅРµ СѓРєР°Р·Р°РЅР°")}</small>`;button.addEventListener("click",()=>selectEpisode(episode).catch(console.error));fragment.appendChild(button);});els.episodesList.appendChild(fragment);decorateEpisodeProgress(release);};
selectEpisode=async function(episode,options={}){if(!state.currentAnime)return;if(!options.preserveSource)state.currentSource="anilibria";state.currentEpisode=episode;syncRenderedEpisodeState();syncRenderedSourceState();showVideoSurface();stopExternalPlayer();const qualities=renderQualityButtons(episode);const selected=qualities.find((q)=>q.key===state.currentQuality)||qualities[0];els.playerTitle.textContent=`${episode.ordinal} СЃРµСЂРёСЏ${episode.name?` вЂў ${episode.name}`:""}`;window.dispatchEvent(new CustomEvent("animecloud:episode-selected",{detail:{release:state.currentAnime,episode,sourceId:state.currentSource}}));if(!selected){destroyPlayer();els.playerNote.textContent="РЈ СЌС‚РѕР№ СЃРµСЂРёРё РїРѕРєР° РЅРµС‚ РґРѕСЃС‚СѓРїРЅРѕРіРѕ РїРѕС‚РѕРєР°.";return;}els.playerNote.textContent=`РџРѕС‚РѕРє Р·Р°РїСѓСЃРєР°РµС‚СЃСЏ С‡РµСЂРµР· РІР°С€ РґРѕРјРµРЅ. РЎС‚Р°СЂС‚РѕРІРѕРµ РєР°С‡РµСЃС‚РІРѕ: ${selected.label}. Р•СЃР»Рё РЅСѓР¶РЅРѕ, РїРµСЂРµРєР»СЋС‡РёС‚Рµ РµРіРѕ РІСЂСѓС‡РЅСѓСЋ.`;try{await attachPlayer(selected.url);els.player.play().catch(()=>{});}catch(error){console.error(error);els.playerNote.textContent="РќРµ СѓРґР°Р»РѕСЃСЊ Р·Р°РіСЂСѓР·РёС‚СЊ РїРѕС‚РѕРє. РџРѕРїСЂРѕР±СѓР№С‚Рµ РґСЂСѓРіРѕРµ РєР°С‡РµСЃС‚РІРѕ РёР»Рё РґСЂСѓРіСѓСЋ СЃРµСЂРёСЋ.";}}; 
switchSource=function(sourceId){if(!state.currentAnime)return;state.currentSource=sourceId;syncRenderedSourceState();window.dispatchEvent(new CustomEvent("animecloud:source-changed",{detail:{release:state.currentAnime,sourceId}}));if(sourceId==="external"&&state.currentAnime.externalPlayer){showExternalSurface(state.currentAnime.externalPlayer);els.qualitySwitch.innerHTML="";els.playerTitle.textContent="Р”СЂСѓРіРёРµ РѕР·РІСѓС‡РєРё";els.playerNote.textContent="Р•СЃР»Рё РІРЅРµС€РЅРёР№ РёСЃС‚РѕС‡РЅРёРє РїРѕРґРґРµСЂР¶РёРІР°РµС‚ AniDub, DEEP, Studio Band РёР»Рё РґСЂСѓРіРёРµ РїРµСЂРµРІРѕРґС‹, РІС‹Р±РёСЂР°Р№С‚Рµ РёС… РІРЅСѓС‚СЂРё СЌС‚РѕРіРѕ РїР»РµРµСЂР°.";return;}if(state.currentEpisode){selectEpisode(state.currentEpisode,{preserveSource:true}).catch(console.error);return;}if(state.currentAnime.episodes.length){selectEpisode(state.currentAnime.episodes[0],{preserveSource:true}).catch(console.error);return;}destroyPlayer();stopExternalPlayer();els.qualitySwitch.innerHTML="";els.playerTitle.textContent="РЎРµСЂРёРё РѕС‚СЃСѓС‚СЃС‚РІСѓСЋС‚";els.playerNote.textContent="Р”Р»СЏ СЌС‚РѕРіРѕ СЂРµР»РёР·Р° РїРѕРєР° РЅРµС‚ РѕРїСѓР±Р»РёРєРѕРІР°РЅРЅС‹С… СЌРїРёР·РѕРґРѕРІ.";};
openRelease=async function(alias,options={}){const payload=await fetchJson(`/anime/releases/${encodeURIComponent(alias)}`,null,{ttl:DETAIL_TTL});const release=buildRelease(payload);state.currentAnime=release;state.currentEpisode=null;state.currentQuality="auto";state.currentSource="anilibria";renderDetails(release);window.dispatchEvent(new CustomEvent("animecloud:release-opened",{detail:{release}}));if(options.updateHash!==false)syncHash(`#anime/${encodeURIComponent(alias)}`);openDrawer();if(release.episodes.length){requestAnimationFrame(()=>selectEpisode(release.episodes[0]).catch(console.error));return;}if(release.externalPlayer){requestAnimationFrame(()=>switchSource("external"));return;}destroyPlayer();stopExternalPlayer();els.qualitySwitch.innerHTML="";els.playerTitle.textContent="РЎРµСЂРёРё РѕС‚СЃСѓС‚СЃС‚РІСѓСЋС‚";els.playerNote.textContent="Р”Р»СЏ СЌС‚РѕРіРѕ СЂРµР»РёР·Р° РїРѕРєР° РЅРµС‚ РѕРїСѓР±Р»РёРєРѕРІР°РЅРЅС‹С… СЌРїРёР·РѕРґРѕРІ.";};
renderProfile=function(){if(!els.favoritesGrid)return;const user=state.authUser;const admin=isAdminUser();els.profileAvatar.src=user?.photoUrl||"./mc-icon-192.png?v=4";els.profileName.textContent=user?.displayName||user?.email?.split("@")[0]||"Р“РѕСЃС‚СЊ";els.profileRoleBadge.hidden=!admin;els.profileEmail.textContent=user?.email||"Р’С…РѕРґ РЅРµ РІС‹РїРѕР»РЅРµРЅ";els.favoritesCount.textContent=formatNumber(state.favorites.length);els.favoritesMode.textContent=admin?"Р’Р»Р°РґРµР»РµС†":user?.localId?"РђРєРєР°СѓРЅС‚":"Р›РѕРєР°Р»СЊРЅРѕ";els.profileSummary.textContent=admin?"Р­С‚Рѕ Р»РѕРєР°Р»СЊРЅС‹Рµ РёРЅСЃС‚СЂСѓРјРµРЅС‚С‹ РІР»Р°РґРµР»СЊС†Р° РґР»СЏ СЌС‚РѕРіРѕ Р±СЂР°СѓР·РµСЂР° Рё СЌС‚РѕР№ СЃР±РѕСЂРєРё СЃР°Р№С‚Р°. Р­С‚Рѕ РЅРµ СЃРµСЂРІРµСЂРЅР°СЏ СЂРѕР»СЊ.":user?.localId?"Р”Р°РЅРЅС‹Рµ РїСЂРѕС„РёР»СЏ СЂР°Р·РґРµР»РµРЅС‹ РїРѕ Р°РєРєР°СѓРЅС‚Р°Рј, РЅРѕ РІСЃС‘ РµС‰С‘ С…СЂР°РЅСЏС‚СЃСЏ Р»РѕРєР°Р»СЊРЅРѕ РІ СЌС‚РѕРј Р±СЂР°СѓР·РµСЂРµ.":"Р‘РµР· РѕР±Р»Р°С‡РЅРѕР№ Р±Р°Р·С‹ РґР°РЅРЅС‹Рµ РёР·Р±СЂР°РЅРЅРѕРіРѕ, РєРѕРјРјРµРЅС‚Р°СЂРёРµРІ Рё РїСЂРѕРіСЂРµСЃСЃР° С…СЂР°РЅСЏС‚СЃСЏ С‚РѕР»СЊРєРѕ РІ СЌС‚РѕРј Р±СЂР°СѓР·РµСЂРµ.";els.adminPanel.hidden=!admin;if(els.adminNote)els.adminNote.textContent=admin?"РРЅСЃС‚СЂСѓРјРµРЅС‚С‹ РІР»Р°РґРµР»СЊС†Р° РґРµР№СЃС‚РІСѓСЋС‚ С‚РѕР»СЊРєРѕ Р»РѕРєР°Р»СЊРЅРѕ Рё РЅРµ Р·Р°РјРµРЅСЏСЋС‚ Р·Р°С‰РёС‰С‘РЅРЅСѓСЋ СЃРµСЂРІРµСЂРЅСѓСЋ Р°РґРјРёРЅРєСѓ.":"РџР°РЅРµР»СЊ РІРёРґРЅР° С‚РѕР»СЊРєРѕ РІР»Р°РґРµР»СЊС†Сѓ РІ СЌС‚РѕР№ Р»РѕРєР°Р»СЊРЅРѕР№ СЃР±РѕСЂРєРµ.";updateGrid(els.favoritesGrid,state.favorites,"Р’ РёР·Р±СЂР°РЅРЅРѕРј РїРѕРєР° РїСѓСЃС‚Рѕ.");renderContinueWatchingSections();};
...
const saveFavoritesBase=saveFavorites;
saveFavorites=function(){saveFavoritesBase();if(state.authUser?.localId&&window.animeCloudSync?.saveFavorites){window.animeCloudSync.saveFavorites(state.authUser,state.favorites).catch(console.error);}};
async function hydrateCloudSessionData(session=state.authUser){if(!session?.localId||!window.animeCloudSync?.hydrateSessionData)return;try{const payload=await window.animeCloudSync.hydrateSessionData(session);if(Array.isArray(payload?.favorites)){state.favorites=payload.favorites;try{localStorage.setItem(favoriteStorageKey(),JSON.stringify(state.favorites));}catch{}}renderProfile();renderFavoriteButton();if(state.currentAnime)renderDetails(state.currentAnime);window.dispatchEvent(new CustomEvent("animecloud:progress-updated",{detail:{hydrated:true}}));}catch(error){console.error(error);}}
window.addEventListener("animecloud:auth",(event)=>{const session=event.detail?.user||null;if(session?.localId){hydrateCloudSessionData(session).catch(console.error);return;}renderContinueWatchingSections();});
safeIdle(()=>{if(state.authUser?.localId)hydrateCloudSessionData(state.authUser).catch(console.error);});
